...
* **Tests.** `python -m pytest -q` runs CPU-only synthetic tests (metrics, bootstrap, calibration, contracts, alignment, spectral encoders, splits, cache round trip, model, losses, engine, figures). They certify code behaviour, not clinical validity.

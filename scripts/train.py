...
encoding = "uniform" if a.config == "B2" else "foveated"
...
train_ds = CacheDataset(reader, train_rows, normalizer); tune_ds = CacheDataset(reader, tune_rows, normalizer) if tune_rows is not None else None
...
write_json(run_dir / "config.resolved.json", {"run_id": rid, "config_id": a.config, "description": DESCRIPTIONS[a.config], "seed": a.seed, "phase": a.phase,
    ...
    "encoding": encoding, "train_partitions": train_parts, "epochs": epochs, "training": cfg, "parameters": params, "git_commit": git_commit(ws.repo),
